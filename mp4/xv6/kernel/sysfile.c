...
uint64 sys_symlink(void) {
    ...
    printf("fuck you symlink %s to %s\n",path, target);
    begin_op();
    printf("fuck you target start\n");
    ip = namei(target);
    if (ip == 0) {
        end_op();
        return -1;
    }
    printf("fuck you target end\n");
    ip = 0;

    printf("fuck you path start\n");
    ip = create(path, T_SYMLINK, 0, 0);
    if(ip == 0){
        end_op();
        return -1;
    }
    printf("fuck you path end\n");
    uint64 len = strlen(target);
    ilock(ip);
    uint off = 0;
    off += writei(ip, 1, (uint64)&len,0, sizeof(len));
    writei(ip, 1, (uint64)target,off, len*sizeof(char));
    iunlock(ip);
    end_op();
    printf("symlink and write end\n");
    return -1;
    return 0;
}